...
```{r}
#Importo los dataset
eph_train_2023 <- read.csv("eph_train_2023.csv", header=TRUE)
# renombro ano4 
...
En cuanto a la significatividad de los coeficientes respecto al modelo anterior, solo se observa un cambio en la significancia de la interacción entre años de educación y sexo, cuyo coeficiente ahora sí es significativo. Esto quiere decir que el efecto de los años de educación no es el mismo para hombres que para mujeres.

El valor del coeficiente estimado que marca la interacción es -0.01154, indicando que, para los varones, el efecto de cada año adicional de educación sobre el salario por hora es 1.154% menor en comparación con el efecto observado en las mujeres por cada año adicional de educación, manteniendo constantes las demás variables.

Además, el coeficiente estimado para la variable sexoVaron es 0.02803, lo cual significa que, a igualdad de educación y experiencia, los varones tienen un salario base 2.8% mayor que las mujeres.

Dado esto, la interpretación del coeficiente asociado a la variable de años de educación tiene dos lecturas:

* Para los varones, el impacto de la educación se ve reducido por el valor del coeficiente de interacción (-0.01154). Por lo tanto, para un varón, el aumento porcentual en el salario por hora por cada año adicional de educación es 9.63% − 1.154% = 8.476%, manteniendo constantes las demás variables. A esto se suma que, a igualdad de condiciones, los varones parten de un salario base 2.8% mayor que el de las mujeres.

* Para las mujeres, cada año adicional de educación está asociado, en promedio, a un aumento del 9.63% en el salario por hora, manteniendo constantes las demás variables.

Esta diferencia entre hombres y mujeres en el impacto de la educación sobre el salario queda clara debido a que tanto el coeficiente de interacción entre educación y sexo como el efecto base de sexoVaron son significativos.
...
```{r}
#levanto el df_test
eph_test_2023 <- read.csv("eph_test_2023.csv", header=TRUE)
...
```{r, message=FALSE}
#levanto el dataset
eph_train_outliers_2023 <- read.csv("eph_train_outliers_2023.csv", header=TRUE)
